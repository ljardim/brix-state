...
namespace Brix.State.Editor {
    [CreateAssetMenu(menuName = Constants.Menus.STATE + "New State Graph")]
    public class StateGraph : ScriptableObject {
        ...
        public bool IsTransitionDuplicate(Node inputNode) {
            var inputTransition = GetNodeById(inputNode.transitionInputNode);
            if (inputTransition == null) {
                return false;
            }

            foreach (var transition in inputTransition.stateRef.currentState.transitions) {
